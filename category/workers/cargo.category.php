...
<div class="main-action-group">
  ...
  <?php echo Jot::btn('begin:plus-square', Config::speak('manager.title_new_', $speak->category), $config->manager->slug . '/category/ignite'); ?>
  <?php Weapon::fire('main_action_after', $hooks); ?>
</div>
<?php echo $messages; ?>
<?php $files_all = Get::state_category(null, array()); ?>
...
<table class="table-bordered table-full-width">
  ...
  <tbody>
    ...
    <tr<?php echo Session::get('recent_item_update') === $key ? ' class="active"' : ""; ?>>
      ...
      <td class="td-icon">
      <?php echo Jot::a('construct', $config->manager->slug . '/category/repair/id:' . $key, Jot::icon('pencil'), array(
          'title' => $speak->edit
      )); ?>
      </td>
      <td class="td-icon">
      <?php echo Jot::a('destruct', $config->manager->slug . '/category/kill/id:' . $key, Jot::icon('times'), array(
          'title' => $speak->delete
      )); ?>
      </td>
      <?php else: ?>
...
<?php else: ?>
<p><?php echo Config::speak('notify_empty', strtolower($speak->categories)); ?></p>
<?php endif; ?>